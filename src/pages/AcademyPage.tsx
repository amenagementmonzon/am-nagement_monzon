...
export default function AcademyPage() {
  ...
  const FILTER_LABELS: Record<string, string> = {
    All: t.academy.filterAll, course: t.academy.filterCourse,
  ...
  return (
    <>
      ...
      <PageShell>
        <PageHero eyebrow={t.academy.pageEyebrow} title={t.academy.pageTitle} subtitle={t.academy.pageSubtitle} />
        ...
        <section className="py-10 bg-charcoal border-b border-white/5">
          <div className="max-w-screen-xl mx-auto px-6 md:px-10">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {[
                { label: t.academy.stat1Label, value: t.academy.stat1  },
                { label: t.academy.stat2Label, value: t.academy.stat2  },
                { label: t.academy.stat3Label, value: t.academy.stat3  },
                { label: t.academy.stat4Label, value: t.academy.stat4  },
              ].map(s => (
          ...
          <div className="max-w-screen-xl mx-auto px-6 md:px-10">
                  ...
                  <div className="flex flex-col flex-1 p-5">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button className="flex items-center gap-1.5 px-4 py-2 bg-charcoal text-gold text-xs font-sans font-semibold rounded-xl hover:bg-gold hover:text-charcoal transition-all duration-300 cursor-pointer">
                        {t.academy.enroll} <ArrowUpRight size={13} weight="bold" />
                      </button>
                    </div>
                  </div>
                </div>
            ...
            {filtered.length === 0 && (
              <div className="text-center py-24">
                ...
                <p className="font-sans text-sm text-gray-400">{t.academy.noItems}</p>
              </div>
            )}
        ...
        <section className="py-16 bg-charcoal">
          <div className="max-w-screen-xl mx-auto px-6 md:px-10 text-center">
            <h2 className="font-headline font-bold text-fluid-xl text-warm-white mb-4">{t.academy.ctaTitle}</h2>
            <p className="font-sans text-sm text-gray-400 max-w-md mx-auto mb-8">{t.academy.ctaSubtitle}</p>
            <a href="/contact" className="inline-flex items-center gap-2 px-8 py-4 bg-gold text-charcoal font-sans font-semibold text-sm rounded-xl hover:bg-gold-dark transition-all shadow-lg shadow-gold/20">
              {t.academy.ctaBtn} <ArrowUpRight size={14} weight="bold" />
            </a>
          </div>
        </section>
      </PageShell>
