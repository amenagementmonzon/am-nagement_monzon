...
function DocumentPreview({ doc, onClose, onApprove, approveLabel, billedToLabel }: {
  doc: any;
  onClose: () => void;
      ...
      <div className="bg-white rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col">
          ...
          <div className="flex items-center gap-2">
            ...
            {doc.type === "estimate" && doc.status === "sent" && onApprove && (
              <button onClick={() => { onApprove(doc.id); onClose(); }}
                ...
                <Check size={11} weight="bold" /> {approveLabel}
              </button>
            )}
        ...
        <div className="flex-1 overflow-auto bg-gray-100 p-6">
          <div className="bg-white shadow-lg max-w-2xl mx-auto p-10" style={{ fontFamily:"sans-serif", color:"#1a1a2e" }}>
            ...
            <div className="mb-8">
              <div className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-1">{billedToLabel}</div>
...
export default function ClientPortalPage() {
  ...
  // Persistent messages via DB
  const portalThreadId = user ? `portal-${user.id}` : "portal-guest";
  const { data: dbMessages, isPending: msgsPending } = useQuery("ChatMessage", {
    where: { threadId: portalThreadId },
    orderBy: { createdAt: "asc" },
  });
  const { create: createMessage } = useMutation("ChatMessage");
  ...
  const NAV_SECTIONS: { id: Section; label: string; icon: React.ElementType }[] = [
    { id: "overview",   label: t.portal.overview,   icon: Briefcase },
    { id: "projects",   label: t.portal.myProjects, icon: FolderOpen },
    { id: "documents",  label: t.portal.documents,  icon: FileText },
    { id: "payments",   label: t.portal.payments,   icon: Receipt },
    { id: "schedule",   label: t.portal.schedule,   icon: CalendarBlank },
    { id: "history",    label: t.portal.activity,   icon: Clock },
    { id: "messages",   label: t.portal.messages,   icon: Chat },
    { id: "profile",    label: t.portal.myProfile,  icon: User },
  ];
  ...
  return (
    <>
            ...
            <div className="min-h-[calc(100vh-72px)] flex items-center justify-center px-6">
              <div className="w-full max-w-md">
                <div className="bg-white border border-gray-200 rounded-3xl p-10 shadow-sm text-center">
                  ...
                  <span className="section-eyebrow text-gold/70 block mb-3">{t.portal.secureAccess}</span>
                  <h1 className="font-headline font-bold text-2xl text-charcoal mb-3">{t.portal.title}</h1>
                  <p className="font-sans text-sm text-gray-500 mb-8 leading-relaxed">{t.portal.subtitle}</p>
                  <button onClick={() => login()}
                    ...
                    <LockKey size={16} weight="bold" /> {t.portal.signInBtn} <ArrowUpRight size={15} weight="bold" />
                  </button>
                  <p className="font-sans text-xs text-gray-400 mt-5">
                    {t.portal.notClient}{" "}
                    <a href="/contact" className="text-gold hover:text-gold-dark transition-colors">{t.portal.requestAccess}</a>
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                {activeSection === "overview" && (
                  <div className="flex flex-col gap-6">
                    <div>
                      <span className="section-eyebrow text-gold/80 block mb-1">{t.portal.title}</span>
                      <h1 className="font-headline font-bold text-2xl text-charcoal">{t.portal.welcomeBack}, {user?.name?.split(" ")[0] ?? t.common.client} 👋</h1>
                      <p className="font-sans text-sm text-gray-500 mt-1">{t.portal.overviewSubtitle}</p>
                    </div>

                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                      {[
                        [t.portal.invoices,  overviewStats.invoices,  "bg-blue-50 text-blue-600"],
                        [t.portal.estimates, overviewStats.estimates, "bg-amber-50 text-amber-600"],
                        [t.portal.paid,      overviewStats.paid,      "bg-emerald-50 text-emerald-600"],
                        [t.portal.pending,   overviewStats.pending,   "bg-red-50 text-red-500"],
                      ].map(([label, val, cls]) => (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      ...
                      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
                        <div className="flex justify-between items-center px-5 py-3.5 border-b border-gray-100">
                          <p className="font-headline font-semibold text-sm text-charcoal">{t.portal.recentDocuments}</p>
                          <button onClick={() => setActiveSection("documents")} className="font-mono text-[10px] text-gold cursor-pointer">{t.portal.viewAll}</button>
                        </div>
                      ...
                      <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
                        <p className="font-headline font-semibold text-sm text-charcoal mb-4">{t.portal.quickActions}</p>
                        <div className="flex flex-col gap-2">
                          {[
                            [t.portal.viewProjects,       "projects"],
                            [t.portal.viewAllInvoices,    "documents"],
                            [t.portal.approveEstimates,   "documents"],
                            [t.portal.paymentHistory,     "payments"],
                            [t.portal.sendMessage,        "messages"],
                          ].map(([label, section]) => (
                ...
                {activeSection === "projects" && (
                  <div className="flex flex-col gap-4">
                    <h2 className="font-headline font-bold text-xl text-charcoal">{t.portal.myProjectsTitle}</h2>
                    ...
                    safeProj.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-16 rounded-2xl border border-dashed border-gray-200 bg-white">
                        ...
                        <p className="font-sans text-sm text-gray-400">{t.portal.noProjects}</p>
                      </div>
                    ) : (
                          ...
                          <div key={p.id} className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                            ...
                            <div className="p-4">
                              <div className="flex items-start justify-between gap-2 mb-2">
                                ...
                                <span className={`flex-shrink-0 px-2 py-0.5 text-[9px] font-mono rounded-full ${p.status === "active" ? "bg-emerald-100 text-emerald-600" : p.status === "completed" ? "bg-blue-100 text-blue-600" : "bg-gray-100 text-gray-500"}`}>{p.status === "active" ? t.common.active : p.status === "completed" ? t.common.completed : (p.status ?? t.common.active)}</span>
                              </div>
                ...
                {activeSection === "documents" && (
                  <div>
                    <h2 className="font-headline font-bold text-xl text-charcoal mb-4">{t.portal.documentsTitle}</h2>
                    <div className="flex flex-wrap gap-2 mb-5">
                      {(["all","estimate","invoice","receipt","work-order","credit-note"] as const).map(dt => (
                        <button key={dt} onClick={() => setDocTypeFilter(dt)}
                          className={`px-3 py-1.5 text-xs font-mono rounded-xl border capitalize transition-all cursor-pointer ${docTypeFilter === dt ? "bg-charcoal text-white border-charcoal" : "bg-white text-gray-500 border-gray-200 hover:border-gray-400"}`}>
                          {dt === "all" ? t.portal.allDocuments : dt === "estimate" ? t.portal.docEstimate : dt === "invoice" ? t.portal.docInvoice : dt === "receipt" ? t.portal.docReceipt : dt === "work-order" ? t.portal.docWorkOrder : t.portal.docCreditNote}
                        </button>
                      ))}
                    </div>
                    {docsLoading ? (
                      ...
                      <div className="flex flex-col items-center justify-center py-16 rounded-2xl border border-dashed border-gray-200 bg-white">
                        ...
                        <p className="font-sans text-sm text-gray-400">{t.portal.noDocuments}</p>
                      </div>
                    ) : (
                      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
                        {safeDocs.map(doc => {
                              ...
                              <div className="flex items-center gap-2 flex-wrap justify-end">
                                ...
                                {doc.type === "estimate" && doc.status === "sent" && (
                                  <button onClick={() => handleApproveEstimate(doc.id)}
                                    className="flex items-center gap-1.5 px-3 py-1.5 bg-teal-50 text-teal-600 border border-teal-200 text-xs font-mono rounded-lg hover:bg-teal-100 cursor-pointer">
                                    <Check size={11} weight="bold" /> {t.portal.approve}
                                  </button>
                                )}
                ...
                {activeSection === "payments" && (
                  <div>
                    <h2 className="font-headline font-bold text-xl text-charcoal mb-4">{t.portal.paymentsTitle}</h2>
                    <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
                      <div className="px-5 py-3.5 bg-gray-50 border-b border-gray-100">
                        <p className="font-mono text-[10px] text-gray-400 uppercase tracking-widest">{t.portal.allPayments}</p>
                      </div>
                      {safeDocs.filter(d => d.type === "invoice" && d.status === "paid").length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-12">
                          ...
                          <p className="font-sans text-sm text-gray-400">{t.portal.noPayments}</p>
                        </div>
                      ) : (
                ...
                {activeSection === "history" && (
                  <div>
                    <h2 className="font-headline font-bold text-xl text-charcoal mb-4">{t.portal.activityTitle}</h2>
                    <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
                      <div className="px-5 py-3.5 bg-gray-50 border-b border-gray-100">
                        <p className="font-mono text-[10px] text-gray-400 uppercase tracking-widest">{t.portal.docStatusActivity}</p>
                      </div>
                      {safeDocs.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-12">
                          <Clock size={24} weight="regular" className="text-gray-300 mb-2" />
                          <p className="font-sans text-sm text-gray-400">{t.portal.noActivity}</p>
                        </div>
                      ) : (
                ...
                {activeSection === "messages" && (
                  <div>
                    <h2 className="font-headline font-bold text-xl text-charcoal mb-4">{t.portal.messagesTitle}</h2>
                    <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm flex flex-col" style={{ height: "500px" }}>
                      <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100 bg-gray-50 flex-shrink-0">
                        ...
                        <div><p className="font-headline font-semibold text-xs text-charcoal">{t.portal.supportName}</p><div className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-green-400" /><span className="font-mono text-[9px] text-gray-400">{t.portal.supportAvailability}</span></div></div>
                      </div>
                      <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-3 scrollbar-hide">
                        {msgsPending ? (
                          <div className="flex items-center justify-center h-full"><span className="w-5 h-5 border-2 border-gold/30 border-t-gold rounded-full animate-spin" /></div>
                        ) : (!dbMessages || dbMessages.length === 0) ? (
                          <div className="flex flex-col items-center justify-center h-full gap-2">
                            <Chat size={28} weight="regular" className="text-gray-300" />
                            <p className="font-sans text-xs text-gray-400 text-center">{t.portal.noMessages}</p>
                          </div>
                        ) : (
                          (dbMessages ?? []).map(msg => {
                            ...
                            return (
                              <div key={msg.id} className={`flex gap-2 ${isClient ? "justify-end" : "justify-start"}`}>
                                {!isClient && <div className="w-7 h-7 rounded-full bg-charcoal flex items-center justify-center flex-shrink-0 mt-0.5"><span className="font-headline font-bold text-[10px] text-gold">M</span></div>}
                                <div className={`max-w-[75%] px-4 py-2.5 rounded-2xl text-xs font-sans leading-relaxed ${isClient ? "bg-charcoal text-warm-white rounded-br-sm" : "bg-gray-100 text-charcoal rounded-bl-sm"}`}>
                                  <p>{msg.message}</p>
                                  <p className="font-mono text-[9px] mt-1 text-gray-400">{new Date(msg.createdAt).toLocaleTimeString("en-CA", { hour: "2-digit", minute: "2-digit" })}</p>
                                </div>
                              </div>
                            );
                          })
                      ...
                      <div className="px-4 py-3 border-t border-gray-100 flex gap-2 items-center flex-shrink-0">
                        ...
                        <button onClick={sendMessage} disabled={!messageDraft.trim()}
                          className="px-4 py-2.5 text-xs font-sans font-semibold bg-charcoal text-white rounded-xl disabled:opacity-40 hover:bg-gray-800 cursor-pointer transition-colors">{t.portal.send}</button>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {activeSection === "schedule" && (
                  <div>
                    <h2 className="font-headline font-bold text-xl text-charcoal mb-4">{t.portal.scheduleTitle}</h2>
                    <p className="font-sans text-sm text-gray-500 mb-5">{t.portal.scheduleSubtitle}</p>
                    {(!calEvents || calEvents.length === 0) ? (
                      <div className="flex flex-col items-center justify-center py-16 rounded-2xl border border-dashed border-gray-200 bg-white">
                        <CalendarBlank size={28} weight="regular" className="text-gray-300 mb-2" />
                        <p className="font-sans text-sm text-gray-400">{t.portal.noSchedule}</p>
                      </div>
                    ) : (
                ...
                {activeSection === "profile" && (
                  <div>
                    <h2 className="font-headline font-bold text-xl text-charcoal mb-4">{t.portal.profileTitle}</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                        <div className="flex items-center justify-between mb-5">
                          <p className="font-headline font-semibold text-sm text-charcoal">{t.portal.accountInfo}</p>
                          <button onClick={() => setEditingProfile(p => !p)}
                            ...
                            <PencilSimple size={12} /> {editingProfile ? t.portal.cancel : t.portal.edit}
                          </button>
                        </div>
                        <div className="flex flex-col gap-4">
                          <div className="flex flex-col gap-1.5">
                            <label className="font-mono text-[10px] text-gray-400 uppercase tracking-widest">{t.portal.fullName}</label>
                            {editingProfile
                          ...
                          <div className="flex flex-col gap-1.5">
                            <label className="font-mono text-[10px] text-gray-400 uppercase tracking-widest">{t.portal.email}</label>
                            <p className="font-sans text-sm text-charcoal">{user?.email}</p>
                            <p className="font-mono text-[9px] text-gray-400">{t.portal.emailNote}</p>
                          </div>
                          <div className="flex flex-col gap-1.5">
                            <label className="font-mono text-[10px] text-gray-400 uppercase tracking-widest">{t.portal.accountType}</label>
                            <span className="px-2.5 py-1 w-fit text-xs font-mono rounded-full bg-blue-100 text-blue-600">{t.common.client}</span>
                          </div>
                          {editingProfile && (
                            <button onClick={() => { setEditingProfile(false); }}
                              ...
                              <FloppyDisk size={13} /> {t.portal.saveChanges}
                            </button>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                        <p className="font-headline font-semibold text-sm text-charcoal mb-5">{t.portal.notifications}</p>
                        <div className="flex flex-col gap-3">
                          {[
                            [t.portal.notifInvoices, true],
                            [t.portal.notifEstimates, true],
                            [t.portal.notifPayments, true],
                            [t.portal.notifSMS, false],
                          ].map(([label, def]) => (
                        ...
                        <div className="mt-6 pt-5 border-t border-gray-100">
                          <button onClick={() => logout()} className="flex items-center gap-2 text-red-400 hover:text-red-600 font-sans text-xs transition-colors cursor-pointer">
                            <SignOut size={14} /> {t.portal.signOut}
                          </button>
                        </div>
                      </div>
                    </div>
      ...
      {previewDoc && <DocumentPreview doc={previewDoc} onClose={() => setPreviewDoc(null)} onApprove={handleApproveEstimate} approveLabel={t.portal.approveEstimate} billedToLabel={t.portal.billedTo} />}
    </>
  );
}
