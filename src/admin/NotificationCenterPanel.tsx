...
export default function NotificationCenterPanel() {
  ...
  const { useLanguage: _ul } = { useLanguage: () => null }; // placeholder—accessed via context directly

  // Dynamic labels via window context (safe in all scenarios)
  const getLang = () => {
    try { return (JSON.parse(localStorage.getItem("monzon-lang-v1") || '"en"') as string); } catch { return "en"; }
  };
