...
const SIDEBAR_ITEM_DEFS = [
  { id: "dashboard",              panelKey: "dashboard",      icon: ChartBar,       group: "core"       },
  { id: "projects",               panelKey: "projects",       icon: Briefcase,      group: "core"       },
  { id: "clients",                panelKey: "clients",        icon: Users,          group: "core"       },
  { id: "employees",              panelKey: "employees",      icon: UserCircle,     group: "core"       },
  { id: "media",                  panelKey: "media",          icon: Images,         group: "core"       },
  { id: "store",                  panelKey: "store",          icon: ShoppingBag,    group: "core"       },
  { id: "community",              panelKey: "community",      icon: Megaphone,      group: "core"       },
  { id: "analytics",              panelKey: "analytics",      icon: ChartBar,       group: "core"       },
  { id: "calendar",               panelKey: "calendar",       icon: Clock,          group: "core"       },
  { id: "messaging",              panelKey: "messaging",      icon: Bell,           group: "core"       },
  { id: "billing",                panelKey: "billing",        icon: CurrencyDollar, group: "finance"    },
  { id: "economics",              panelKey: "economics",      icon: TrendUp,        group: "finance"    },
  { id: "expenses",               panelKey: "expenses",       icon: ChartBar,       group: "finance"    },
  { id: "leads",                  panelKey: "leads",          icon: FunnelSimple,   group: "finance"    },
  { id: "hours",                  panelKey: "hours",          icon: Clock,          group: "finance"    },
  { id: "profitability",          panelKey: "profitability",  icon: Calculator,     group: "finance"    },
  { id: "ai-admin",               panelKey: "aiAnalyst",      icon: Robot,          group: "ai"         },
  { id: "theme",                  panelKey: "theme",          icon: Palette,        group: "appearance" },
  { id: "appearance-base",        panelKey: "appearanceBase", icon: Monitor,        group: "appearance" },
  { id: "splash-manager",         panelKey: "splash",         icon: Play,           group: "appearance" },
  { id: "video-manager",          panelKey: "video",          icon: Video,          group: "appearance" },
  { id: "reviews",                panelKey: "reviews",        icon: Star,           group: "content"    },
  { id: "metrics",                panelKey: "metrics",        icon: TrendUp,        group: "content"    },
  { id: "logo-manager",           panelKey: "logo",           icon: Images,         group: "content"    },
  { id: "company-profile",        panelKey: "company",        icon: Buildings,      group: "content"    },
  { id: "academy-manager",        panelKey: "academy",        icon: FileText,       group: "content"    },
  { id: "service-shop",           panelKey: "serviceShop",    icon: Briefcase,      group: "content"    },
  { id: "roles",                  panelKey: "roles",          icon: ShieldCheck,    group: "settings"   },
  { id: "users",                  panelKey: "users",          icon: UserGear,       group: "settings"   },
  { id: "integrations",           panelKey: "integrations",   icon: Plug,           group: "settings"   },
  { id: "notifications-center",   panelKey: "notifications",  icon: BellRinging,    group: "settings"   },
  { id: "notifications-settings", panelKey: "notifSettings",  icon: Bell,           group: "settings"   },
  { id: "pwa",                    panelKey: "pwa",            icon: DeviceMobile,   group: "settings"   },
  { id: "settings",               panelKey: "settings",       icon: Gear,           group: "settings"   },
  { id: "i18n",                   panelKey: "i18n",           icon: Gear,           group: "settings"   },
] as const;
...
function DashboardPanel({ setActiveTab }: { setActiveTab: (t: string) => void }) {
  ...
  const stats = [
    { label: t.admin.dashboard.activeProjects, value: String(projects?.length ?? "—")   },
    { label: t.admin.dashboard.clients,        value: String(clients?.length ?? "—")     },
    { label: t.admin.dashboard.employees,      value: String(employees?.length ?? "—")   },
    { label: t.admin.dashboard.mediaFiles,     value: String(mediaAssets?.length ?? "—") },
  ];

  return (
    <div>
      <div className="mb-8">
        <h1 className="font-headline font-bold text-2xl text-charcoal">{t.admin.dashboard.title}</h1>
        <p className="font-sans text-sm text-gray-500 mt-1">{t.admin.dashboard.subtitle}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="bg-white border-gray-200 shadow-sm">
          <CardHeader><CardTitle className="font-headline text-base text-charcoal">{t.admin.dashboard.quickActions}</CardTitle></CardHeader>
          <CardContent>
            <div className="flex flex-col gap-2">
              {([
                [t.admin.dashboard.addProject,"projects"],
                [t.admin.dashboard.addClient,"clients"],
                [t.admin.dashboard.uploadMedia,"media"],
                [t.admin.dashboard.viewAnalytics,"analytics"],
              ] as [string,string][]).map(([label, tab]) => (
        ...
        <Card className="bg-white border-gray-200 shadow-sm">
          <CardHeader><CardTitle className="font-headline text-base text-charcoal">{t.admin.dashboard.recentContacts}</CardTitle></CardHeader>
          <CardContent>
            <ContactsPreview />
          </CardContent>
        </Card>
...
function ContactsPreview() {
  ...
  if (!submissions || submissions.length === 0) return <p className="font-sans text-sm text-gray-400 text-center py-4">{t.admin.dashboard.noSubmissions}</p>;
  return (
...
function I18nManagerPanel() {
  ...
  // Reload overrides when lang changes
  useEffect(() => {
    try {
      const stored = JSON.parse(localStorage.getItem(`monzon-i18n-overrides-${lang}`) || "{}");
      setOverrides(stored);
    } catch { setOverrides({}); }
  }, [lang]);

  const SECTIONS = Object.keys(t.admin.i18n.sections);

  // Flatten current section's strings into {key: value} pairs
  const currentTranslations = t[activeSection as keyof typeof t];
  const flatPairs = (() => {
    const pairs: [string, string][] = [];
    const flatten = (obj: any, prefix = "") => {
      Object.entries(obj).forEach(([k, v]) => {
        const fullKey = prefix ? `${prefix}.${k}` : k;
        if (typeof v === "string") pairs.push([fullKey, v]);
        else if (Array.isArray(v)) v.forEach((item, i) => { if (typeof item === "string") pairs.push([`${fullKey}[${i}]`, item]); });
        else if (typeof v === "object" && v !== null) flatten(v, fullKey);
      });
    };
    if (currentTranslations && typeof currentTranslations === "object") flatten(currentTranslations);
    return pairs.filter(([k, v]) => !search || k.toLowerCase().includes(search.toLowerCase()) || v.toLowerCase().includes(search.toLowerCase()));
  })();
  ...
  const handleSave = () => {
    localStorage.setItem(`monzon-i18n-overrides-${lang}`, JSON.stringify(overrides));
    setSaved(true);
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="font-headline font-bold text-2xl text-charcoal">{t.admin.i18n.title}</h1>
        <p className="font-sans text-sm text-gray-500 mt-1">{t.admin.i18n.subtitle}</p>
      </div>

      {/* Language + section selectors */}
      <div className="flex flex-wrap items-center gap-3 mb-6">
        <div className="flex gap-1 p-1 bg-gray-100 rounded-xl">
          {languages.map((l) => (
            <button key={l.code} onClick={() => setLang(l.code)} className={`px-4 py-2 text-sm font-sans font-medium rounded-lg transition-all cursor-pointer flex items-center gap-1.5 ${lang === l.code ? "bg-white text-charcoal shadow-sm" : "text-gray-400"}`}>
              {l.flag} {l.label}
            </button>
          ))}
        </div>
        <Input
          ...
          className="border-gray-200 rounded-xl text-sm max-w-xs"
        />
        <div className="flex items-center gap-2 ml-auto">
          {saved && <span className="text-xs text-green-600 font-sans font-medium">{t.admin.i18n.saved}</span>}
          <Button onClick={handleSave} className="bg-gold text-charcoal hover:bg-gold-dark text-xs h-9 px-5 rounded-xl shadow-none">
            {t.admin.i18n.saveSection}
          </Button>
        </div>
      </div>
          ...
          <div className="flex flex-col gap-0.5">
            {SECTIONS.map((section) => (
              <button key={section} onClick={() => setActiveSection(section)}
                className={`px-3 py-2 text-xs font-sans font-medium rounded-lg text-left transition-all cursor-pointer w-full ${activeSection === section ? "bg-gold/10 text-gold" : "text-gray-500 hover:bg-gray-100"}`}>
                {t.admin.i18n.sections[section] ?? section}
              </button>
            ))}
          </div>
        </div>
            ...
            <CardContent className="p-0">
              {flatPairs.length === 0 ? (
                <div className="py-10 text-center">
                  <p className="font-sans text-sm text-gray-400">{t.admin.i18n.noResults}</p>
                </div>
              ) : (
                <div className="divide-y divide-gray-50">
                  {flatPairs.map(([key, defaultValue]) => (
                    <div key={key} className="px-5 py-3 flex flex-col gap-1.5">
                      <p className="font-mono text-[10px] text-gray-400 uppercase tracking-wider">{key}</p>
                      <Input
                        defaultValue={overrides[key]?.value ?? defaultValue}
                        onBlur={(e) => handleOverride(key, e.target.value)}
                        className="border-gray-200 rounded-xl text-sm"
                      />
                    </div>
                  ))}
...
function SettingsPanel({ setActiveTab }: { setActiveTab: (t: string) => void }) {
  ...
  const QUICK_LINKS = [
    { label: t.admin.panels.pwa,           tab: "pwa",                    icon: DeviceMobile, desc: "Install, caching, service worker" },
    { label: t.admin.panels.integrations,  tab: "integrations",           icon: Plug,         desc: "Email, AI, media, webhooks, API keys" },
    { label: t.admin.panels.notifications, tab: "notifications-center",   icon: BellRinging,  desc: "Notification inbox" },
    { label: t.admin.panels.notifSettings, tab: "notifications-settings", icon: Bell,         desc: "Channels, roles, frequency" },
    { label: t.admin.panels.roles,         tab: "roles",                  icon: ShieldCheck,  desc: "RBAC and permissions" },
    { label: t.admin.panels.users,         tab: "users",                  icon: UserGear,     desc: "User roles and access" },
    { label: t.admin.panels.i18n,          tab: "i18n",                   icon: Gear,         desc: "Manage EN / FR / ES translations" },
  ];

  return (
    <div>
      <div className="mb-8">
        <h1 className="font-headline font-bold text-2xl text-charcoal">{t.admin.settings.title}</h1>
        <p className="font-sans text-sm text-gray-500 mt-1">{t.admin.settings.subtitle}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <Card className="bg-white border-gray-200 shadow-sm">
          <CardHeader>
            <CardTitle className="font-headline text-base text-charcoal">{t.admin.settings.siteIdentity}</CardTitle>
            <CardDescription className="font-sans text-xs text-gray-400">{t.admin.settings.siteIdentityDesc}</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex flex-col gap-4">
              {([
                { id: "siteTitle", label: t.admin.settings.siteTitle },
                { id: "siteDesc",  label: t.admin.settings.metaDesc },
                { id: "email",     label: t.admin.settings.adminEmail },
                { id: "phone",     label: t.admin.settings.phone },
                { id: "address",   label: t.admin.settings.address },
                { id: "instagram", label: t.admin.settings.instagram },
                { id: "facebook",  label: t.admin.settings.facebook },
        ...
        <div className="flex flex-col gap-3">
          <p className="font-mono text-[10px] text-gray-400 uppercase tracking-widest px-1">{t.admin.settings.systemModules}</p>
      ...
      <div className="min-h-screen bg-charcoal flex items-center justify-center px-6">
        <div className="w-full max-w-md">
          <div className="bg-surface-2 border border-gray-700/60 rounded-3xl p-10 text-center shadow-2xl">
            ...
            <h1 className="font-headline font-bold text-xl text-warm-white mb-2">{t.admin.signIn}</h1>
            <p className="font-sans text-sm text-gray-400 mb-8">{t.admin.signInDesc}</p>
            <button onClick={() => login()}
              ...
              <LockKey size={16} weight="bold" /> {t.admin.signInBtn}
            </button>
            <p className="font-sans text-xs text-gray-600 mt-4">{t.admin.adminEmail} silviolmonzon@amenagementmonzon.com</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-charcoal flex items-center justify-center px-6">
        <div className="w-full max-w-md">
          <div className="bg-surface-2 border border-gray-700/60 rounded-3xl p-10 text-center shadow-2xl">
            ...
            <h1 className="font-headline font-bold text-xl text-warm-white mb-2">{t.admin.accessRestricted}</h1>
            <p className="font-sans text-sm text-gray-400 mb-8">{t.admin.accessRestrictedDesc.replace("{email}", user?.email ?? "")}</p>
            <div className="flex flex-col gap-2">
    ...
    <div className="min-h-screen bg-gray-50 font-sans flex flex-col">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-charcoal border-b border-gray-700 h-14 flex items-center px-5 gap-4">
        ...
        <span className="font-headline font-bold text-sm text-warm-white">
          ...
          <span className="ml-2 font-mono text-[9px] text-gray-500 bg-gray-800 px-1.5 py-0.5 rounded tracking-widest uppercase">{t.admin.panels.dashboard.split(" ")[0] === "Tableau" ? "Admin" : "Admin"}</span>
        </span>
        ...
        <Link to="/" className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-sans font-medium bg-surface-2 text-gray-300 rounded-lg border border-gray-700 hover:border-gold/40 hover:text-warm-white transition-all duration-200 focus:outline-none">
          <Eye size={13} weight="regular" /> {t.admin.viewSite}
        </Link>
      ...
      <div className="flex pt-14 min-h-screen">
        ...
        <aside className={`fixed top-14 bottom-0 left-0 z-40 bg-charcoal border-r border-gray-700/60 flex flex-col transition-all duration-300 ${sidebarOpen ? "w-52" : "w-14"} overflow-hidden`}>
          <nav className="flex-1 py-3 flex flex-col gap-0 px-2 overflow-y-auto" aria-label="Admin navigation">
            {(["core","finance","content","ai","appearance","settings"] as const).map((group) => {
              const items = SIDEBAR_ITEM_DEFS.filter((i) => i.group === group);
              return (
                <div key={group} className="mb-1">
                  ...
                  <div className="flex flex-col gap-0.5">
                    {items.map(({ id, panelKey, icon: Icon }) => {
                      const label = t.admin.panels[panelKey as keyof typeof t.admin.panels] ?? panelKey;
                      return (
        ...
        <main className={`flex-1 transition-all duration-300 overflow-auto ${sidebarOpen ? "ml-52" : "ml-14"}`}>
          <div className="max-w-5xl mx-auto px-6 py-8">
            ...
            {activeTab === "i18n"                 && <I18nManagerPanel />}
