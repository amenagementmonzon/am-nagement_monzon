...
export default function Footer() {
  ...
  const LINKS = {
    ...
    [t.footer.exploreGroup]: [
      { label: t.footer.links.smStore,       href: "/store"        },
      { label: t.footer.links.smCollection,  href: "/store"        },
      { label: t.footer.links.community,     href: "/community"    },
      { label: t.footer.links.maintenance,   href: "/maintenance"  },
      ...
      <div className="relative z-10 max-w-screen-xl mx-auto px-6 md:px-10 pt-16 pb-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-10 mb-12">
          ...
          <div className="lg:col-span-2">
            <Link to="/" className="flex items-center gap-2.5 mb-5 group focus:outline-none">
              ...
              >
                <img
                  src={activeFooterLogo?.url || "https://c.animaapp.com/mmr3v89y52bO3P/img/uploaded-asset-1773625307363-0.png"}
        ...
        <div className="border-t pt-10 mb-10" style={{ borderColor }}>
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div>
              <h3 className="font-headline font-bold text-base mb-1" style={{ color: "hsl(36,20%,97%)" }}>{t.footer.stayUpdated}</h3>
              <p className="font-sans text-sm" style={{ color: textColor, opacity: 0.55 }}>{t.footer.stayUpdatedDesc}</p>
            </div>
            {subDone ? (
              ...
              <form onSubmit={handleSub} className="flex gap-2 w-full md:w-auto">
                <input
                  type="email" placeholder={t.footer.emailPlaceholder} value={email}
                ...
                >
                  {t.footer.subscribe} <ArrowUpRight size={14} weight="bold" />
                </button>
              </form>
            )}
          </div>
        ...
        <div className="border-t pt-8 flex flex-col md:flex-row items-center justify-between gap-4" style={{ borderColor }}>
          <p className="font-sans text-xs" style={{ color: textColor, opacity: 0.4 }}>© {new Date().getFullYear()} Aménagement Monzon. {t.footer.copyright}</p>
          <div className="flex items-center gap-6">
            ...
            >{t.footer.ourStory}</Link>
            <Link to="/academy" className="font-sans text-xs transition-colors duration-200"
              style={{ color: textColor, opacity: 0.4 }}
              onMouseEnter={e => (e.currentTarget as HTMLElement).style.color = accentColor}
              onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = textColor ?? ""; (e.currentTarget as HTMLElement).style.opacity = "0.4"; }}
            >{t.footer.academy}</Link>
            <p className="font-mono text-xs" style={{ color: textColor, opacity: 0.25 }}>{t.footer.craftedIn}</p>
          </div>
        </div>
      </div>
    </footer>
