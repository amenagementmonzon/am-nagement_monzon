...
const FALLBACK_PRODUCTS = [
  { image: "https://c.animaapp.com/mmr3v89y52bO3P/img/ai_2.png", name: "Monzon Signature Tee",   price: "$45",  category: "Apparel" },
  { image: "https://c.animaapp.com/mmr3v89y52bO3P/img/ai_4.png", name: "Premium Tool Set",       price: "$120", category: "Tools" },
  { image: "https://c.animaapp.com/mmr3v89y52bO3P/img/ai_3.png", name: "Brand Accessory Kit",    price: "$65",  category: "Accessories" },
  { image: "https://c.animaapp.com/mmr3v89y52bO3P/img/ai_5.png", name: "Field Workwear Jacket",  price: "$155", category: "Apparel" },
];
    ...
    <div ref={cardRef} className="group relative flex flex-col bg-white border border-gray-200 rounded-2xl overflow-hidden card-cinematic hover:border-gray-300 hover:shadow-xl hover:shadow-black/5">
      <div className="relative overflow-hidden aspect-[4/3]">
        ...
        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
          <span className="font-sans text-xs text-warm-white font-medium tracking-wider uppercase">{t.store.quickView}</span>
        </div>
      </div>
      <div className="flex items-center justify-between p-5 gap-4">
        ...
        <button onClick={() => { if (!added) { setAdded(true); setTimeout(() => setAdded(false), 2200); } }}
          ...
          aria-label={added ? t.store.addedToCart : `Add ${product.name} to cart`}>
      ...
      <div className="max-w-screen-xl mx-auto px-6 md:px-10">
        <div ref={headerRef} className="flex flex-col md:flex-row md:items-end md:justify-between gap-8 mb-12">
          <div>
            ...
            <h2 className="font-headline font-bold text-fluid-2xl text-charcoal leading-[1.1]">{t.store.title}</h2>
            <p className="font-sans text-sm text-gray-500 mt-4 max-w-md leading-relaxed">{t.store.subtitle}</p>
          </div>
          <Link to="/store"
        ...
        <div className="mt-12 rounded-2xl bg-gradient-1 border border-white/5 p-8 flex flex-col sm:flex-row items-center justify-between gap-6">
          <div>
            <p className="font-headline font-bold text-lg text-warm-white">{t.store.freeShipping}</p>
            <p className="font-sans text-sm text-gray-400 mt-1">{t.store.freeShippingDesc}</p>
          </div>
          <Link to="/store"
            ...
            {t.store.shopNow} <ArrowUpRight size={14} weight="bold" className="text-gold" />
          </Link>
        </div>
      </div>
    </section>
