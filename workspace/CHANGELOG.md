...
- `academy` + `maintenance` + `community` sections added — all public pages now 100% translated
- `AboutPage`, `BlogPage`, `PortfolioPage`, `ServicesPage`, `AcademyPage`, `MaintenancePage`, `CommunityPage` all wired to `useLanguage()`
- `AdminShell.tsx`: `SIDEBAR_ITEM_DEFS` array replaces `SIDEBAR_ITEMS`; sidebar group labels, panel titles, auth gate, dashboard, settings all read from `t.admin.*`; `LanguageSelector` added to top bar; `I18nManagerPanel` inline component added allowing admins to browse and override any translation key per language (saved to localStorage)

## 2026-03-16 — Full i18n Coverage: Footer, Header Dropdown, Store, Splash
- `Footer.tsx`: `useLanguage` injected; LINKS object moved inside component and built from `t.footer.*`; tagline, stayUpdated, subscribe, copyright, ourStory, academy, craftedIn all use translation keys
- `HeaderNav.tsx`: hardcoded `"Services"` in dropdown button replaced with `t.nav.servicesDropdown`
- `StorePage.tsx`: `useLanguage` added to `ProductCard`, `PaymentModal`, `CartDrawer`, `StorePage`; all visible strings (tabs, cart, checkout, payment steps, card labels, error messages, wire/interac/cash details, done screen) now use `t.store.*`
- `SplashScreen.tsx`: added `handleLogoClick` — clicking the assembled MonzonLogo wordmark dismisses splash and navigates to `/`; sub-question now uses `t.nav.whichNeeds`; welcome eyebrow adds "Bienvenido"

## 2026-03-16 — Full Trilingual System (EN / FR / ES) with Single-Button Selector
- New `src/i18n/translations.ts`: full translation map for EN/FR/ES covering nav, portal (login gate, all 8 sections, document preview), and common labels
- New `src/contexts/LanguageContext.tsx`: `LanguageProvider` with localStorage + browser language auto-detect, `useLanguage()` hook
- New `src/components/LanguageSelector.tsx`: compact Globe+flag toggle button, 3-item dropdown with active dot and dark/light header modes
- `src/index.tsx`: `LanguageProvider` injected between ThemeProvider and AnimationProvider
- `src/components/HeaderNav.tsx`: `LanguageSelector` inserted in desktop CTA row and mobile drawer footer; nav labels (Sign In/Register/Admin/Portal/CTA) now use `t.nav.*`
- `src/pages/ClientPortalPage.tsx`: every user-visible string replaced with `t.portal.*` / `t.common.*`; `DocumentPreview` accepts translated `approveLabel`/`billedToLabel` props

## 2026-03-16 — Fix: Client Portal Messages Now Persist to DB
- `ClientPortalPage.tsx`: `sendMessage` now calls `createMessage` (useMutation ChatMessage) instead of local state only
- Thread ID is `portal-{user.id}`, senderType `"client"` — matches MessagingPanel's clientThreads filter
- Message list replaced with live `useQuery("ChatMessage", { where: { threadId } })` — shows admin replies in real time
- Removed local `messages` state array entirely; empty state shows a proper placeholder

## 2026-03-16 — Integrations Panel — Full Production Scaffold (7 Modules)
- `IntegrationsPanel.tsx` fully rebuilt: 6 integration modules (Supabase, Groq AI, Gmail SMTP, YouTube/Vimeo, 3D Asset Storage, Image Library) + Webhooks + Env Vars Reference
- Each module has sectioned accordion fields, password show/hide, textarea/select types, env variable badges, configured-field counter, doc link, and deployment-readiness notice
- Webhooks: 6 pre-wired endpoints (Zapier/Lead, Slack/Invoice, QuickBooks/Payment, Slack/Project, Zapier/Contact, Custom), event selector, signing secret, retry count, + add/remove; payload format doc inline
- Env Vars Reference card lists all ~65 env keys across all modules with VITE_ vs server-side distinction and production tip
- Zero external calls made; all data persisted to localStorage; no router changes
